...
import { t, htmlLangAttr } from './i18n.js';
...
const generatedGuideRecordKey = (lang, slug) => `${lang}:${slug}`;
const generatedGuidesByLangSlug = new Map();
const generatedGuideSummariesByLang = { cn: [], en: [] };
...
const staticGuideCatalogByLang = { cn: [], en: [] };
const staticGuideSummariesByLangSlug = new Map();
...
const escapeHtml = (value) => String(value ?? '')
    ...
    .replace(/"/g, '&quot;')
    .replace(/'/g, '&#39;');
const trim = (s, max) => s.length <= max ? s : s.slice(0, max - 1).trimEnd() + '…';
const sanitizeGeneratedGuideBody = (bodyHtml) => sanitizeHtml(bodyHtml, {
...
const getContent = (lang) => (lang === 'cn' ? CONTENT_ZH : CONTENT_EN);
const langPrefix = (lang) => `/${lang}`;
const buildCanonicalUrl = (route, lang) => {
    const pathPart = route ? `/${route}/` : '/';
    return `${SITE}/${lang}${pathPart}`;
};
const langPath = (route, lang) => route ? `${langPrefix(lang)}/${route}` : langPrefix(lang);
const getStaticGuideSummary = (slug, lang) => slug ? staticGuideSummariesByLangSlug.get(generatedGuideRecordKey(lang, slug)) : undefined;
const uniqueNonEmptyValues = (values) => Array.from(new Set(values.map((value) => String(value ?? '').trim()).filter(Boolean)));
// ----------------------------- SEO config lookup ----------------------------- //
const seoPagesByCanonical = new Map();
for (const p of (seoSop?.pages ?? [])) {
    seoPagesByCanonical.set(p.url.replace(/\/$/, ''), p);
}
function lookupSeoPage(route, lang) {
...
const linkList = (items) => `<ul>${items
    .map((it) => `<li><a href="${escapeHtml(it.href)}">${escapeHtml(it.label)}</a></li>`)
    .join('')}</ul>`;
const dlList = (pairs) => `<dl>${pairs
    .map((p) => `<dt>${escapeHtml(p.label)}</dt><dd>${escapeHtml(p.value)}</dd>`)
    .join('')}</dl>`;
const ul = (items) => `<ul>${items.map((i) => `<li>${escapeHtml(i)}</li>`).join('')}</ul>`;
const faqHtml = (lang, items) => {
    if (!items.length)
        return '';
    return `
    <section class="seo-faq">
      ...
      <dl>
        ${items
        .map((it) => `<dt><strong>Q:</strong> ${escapeHtml(it.q)}</dt><dd><strong>A:</strong> ${escapeHtml(it.a)}</dd>`)
        .join('')}
      </dl>
    </section>`;
};
const breadcrumbHtml = (lang, items) => {
    if (items.length === 0)
        return '';
    return `<nav aria-label="breadcrumb" class="seo-breadcrumb">${items
        .map((b, i) => i === items.length - 1
        ? `<span aria-current="page">${escapeHtml(b.label)}</span>`
        : `<a href="${escapeHtml(b.href)}">${escapeHtml(b.label)}</a> <span aria-hidden="true">›</span> `)
        .join('')}</nav>`;
};
const geoLine = (lang, geoTargets) => `<p class="seo-geo"><strong>${escapeHtml(t('servedMarkets', lang))}:</strong> ${escapeHtml(geoTargets.join(', '))}</p>`;
// ----------------------------- Schema builders ----------------------------- //
...
// ----------------------------- Breadcrumbs ----------------------------- //
function buildBreadcrumb(route, lang) {
    ...
    // route taxonomy
    if (parts[0] === 'about')
        return [home, { label: t('about', lang), href: langPath('about', lang) + '/' }];
    if (parts[0] === 'culture')
        return [home, { label: t('culture', lang), href: langPath('culture', lang) + '/' }];
    if (parts[0] === 'tour')
        return [home, { label: t('tour', lang), href: langPath('tour', lang) + '/' }];
    if (parts[0] === 'quote')
        return [home, { label: t('quote', lang), href: langPath('quote', lang) + '/' }];
    if (parts[0] === 'privacy')
        return [home, { label: t('privacy', lang), href: langPath('privacy', lang) + '/' }];
    if (parts[0] === 'terms')
        return [home, { label: t('terms', lang), href: langPath('terms', lang) + '/' }];
    if (parts[0] === 'pintefoils')
        return [home, { label: t('pintefoils', lang), href: langPath('pintefoils', lang) + '/' }];
    if (parts[0] === 'seo-geo-sop')
        return [home, { label: t('seoSop', lang), href: langPath('seo-geo-sop', lang) + '/' }];
    if (parts[0] === 'guides') {
    ...
    if (parts[0] === 'blog') {
        ...
        if (parts[1])
            crumbs.push({ label: parts[1], href: langPath(route, lang) + '/' });
        return crumbs;
    }
    if (parts[0] === 'products') {
        const crumbs = [
        ...
        if (parts[1] === 'foils') {
            ...
            const cat = getContent(lang).PRODUCT_DATA[parts[2]];
            crumbs.push({
            ...
            for (const [seriesId, items] of Object.entries(catalog)) {
                const m = items.find((x) => x.id === parts[2]);
                if (m) {
                    foundItem = m;
            ...
            if (foundSeries) {
                const cat = getContent(lang).PRODUCT_DATA[foundSeries];
                crumbs.push({
    ...
    if (parts[0] === 'solutions') {
        ...
        if (parts[1]) {
            const sol = getContent(lang).SOLUTIONS_DATA[parts[1]];
            crumbs.push({
...
function crumbsToSchema(crumbs) {
    return generateBreadcrumbSchema(crumbs.map((c) => ({ name: c.label, url: `${SITE}${c.href}` })));
}
// ----------------------------- Common assemblers ----------------------------- //
function wrapMain(opts) {
    return `<main class="seo-snapshot" lang="${htmlLangAttr(opts.lang)}" data-route="${escapeHtml(langPath(opts.route, opts.lang))}">${opts.breadcrumb}${opts.inner}</main>`;
}
function defaultGeoForRoute(route, lang) {
    const seo = lookupSeoPage(route, lang);
    return seo?.geoTargets?.length ? seo.geoTargets : defaultGeoTargets;
}
function defaultKeywords(route, lang, extras = []) {
    const seo = lookupSeoPage(route, lang);
    const list = seo
...
// ----------------------------- Per-route builders ----------------------------- //
function buildHomeSnapshot(lang) {
    ...
    const painPoints = Array.from(new Set(Object.values(c.SOLUTIONS_DATA).flatMap((s) => s.painPoints || [])));
    const painLead = lang === 'cn'
        ? `烫金膜在生产中常见痛点 — ${painPoints.join('、')} — 品特针对每一类痛点都开发了对应的产品系列与解决方案,经过 25 年市场验证。`
        : `Common pain points in hot stamping production — ${painPoints.join(', ')} — are each addressed by a dedicated PINTE product series, validated over 25 years.`;
    const faq = [
        {
        ...
        {
            q: t('qMarkets', lang),
            a: (lang === 'cn' ? '品特长期服务: ' : 'PINTE primarily serves: ') +
                geoTargets.join(', ') +
                (lang === 'cn'
...
function buildCategorySnapshot(catId, lang) {
    const c = getContent(lang);
    const route = `products/category/${catId}`;
    const url = buildCanonicalUrl(route, lang);
    const cat = c.PRODUCT_DATA[catId];
    const items = c.CATALOG_DATA[catId] || [];
    const solutions = Object.values(c.SOLUTIONS_DATA).filter((s) => s.series === catId);
    const geoTargets = defaultGeoForRoute(route, lang);
    ...
    const title = categoryTitleMap[catId]?.[lang] ||
        (lang === 'cn'
    ...
    const painPoints = Array.from(new Set(solutions.flatMap((s) => s.painPoints || [])));
    const painLead = painPoints.length === 0
        ? cat.description
        : lang === 'cn'
    ...
    <section>
      ...
      <ul>
        ${features
        .map((f) => `<li><strong>${escapeHtml(f.title)}:</strong> ${escapeHtml(f.desc)}</li>`)
        .join('')}
      </ul>
    </section>`;
    const structureHtml = `
    ...
    <section>
      ...
      ${dlList(cat.params || [])}
      ${cat.temp
        ? `<p><strong>${escapeHtml(t('temperature', lang))}</strong> — ${escapeHtml(t('flatTemp', lang))}: ${escapeHtml(cat.temp.flat)}; ${escapeHtml(t('roundTemp', lang))}: ${escapeHtml(cat.temp.round)}</p>`
        : ''}
    </section>
...
function buildItemSnapshot(itemId, lang) {
    const c = getContent(lang);
    ...
    const cat = c.PRODUCT_DATA[seriesId];
    const sameSeries = c.CATALOG_DATA[seriesId].filter((x) => x.id !== itemId);
    const relatedSolutions = Object.values(c.SOLUTIONS_DATA).filter((s) => s.series === seriesId);
    const geoTargets = defaultGeoForRoute(route, lang);
    const title = lang === 'cn'
    ...
    ${params.length
        ...
        ${dlList(params)}
        ${item.temp
            ? `<p><strong>${escapeHtml(t('temperature', lang))}</strong> — ${escapeHtml(t('flatTemp', lang))}: ${escapeHtml(item.temp.flat)}; ${escapeHtml(t('roundTemp', lang))}: ${escapeHtml(item.temp.round)}</p>`
            : ''}
      </section>`
        : ''}
    ...
    ${technicalParameters.length || qualityTests.length || item.moq || item.samplePolicy
        ...
        ${item.customizationLeadTime
            ? `<p><strong>${lang === 'cn' ? '定制周期' : 'Customization'}:</strong> ${escapeHtml(item.customizationLeadTime)}</p>`
            : ''}
      </section>`
        : ''}
    ...
    <section>
      <h2>${escapeHtml(t('related', lang))}</h2>
      <p>${escapeHtml(t('partOfSeries', lang, { series: cat.name }))} — <a href="${escapeHtml(langPath(`products/category/${seriesId}`, lang) + '/')}">${escapeHtml(t('viewSeries', lang, { series: cat.name }))}</a></p>
      ${sameSeries.length
        ? linkList(sameSeries.map((x) => ({
            label: x.name,
            href: langPath(`products/item/${x.id}`, lang) + '/',
        })))
        : ''}
      ${relatedSolutions.length
        ? linkList(relatedSolutions.map((s) => ({
            label: s.title,
            href: langPath(`solutions/${s.id}`, lang) + '/',
        })))
        : ''}
  ...
  `;
    ...
    const jsonLd = [
        ...
        productSchema({
            ...
            applications,
            properties: [
                ...params.map((param) => ({ name: param.label, value: param.value })),
                ...specifications.map((param) => ({ name: param.label, value: param.value })),
                ...technicalParameters.map((param) => ({ name: param.label, value: param.value })),
                ...(item.temp
                    ? [
...
function buildSolutionSnapshot(solId, lang) {
    ...
    const faq = [
        ...
        ...features.slice(0, 2).map((f) => ({
            ...
            a: (cat?.description ? trim(cat.description, 300) : sol.description),
        })),
        { q: t('qOEM', lang), a: t('aOEM', lang) },
        { q: t('qLeadTime', lang), a: t('aLeadTime', lang) },
    ];
    ...
    ${cat
        ? `<section>
        <h2>${escapeHtml(t('related', lang))}</h2>
        <p><a href="${escapeHtml(langPath(`products/category/${seriesId}`, lang) + '/')}">${escapeHtml(t('viewSeries', lang, { series: cat.name }))}</a></p>
        ${sameSeriesItems.length
            ? linkList(sameSeriesItems.map((it) => ({
                label: it.name,
                href: langPath(`products/item/${it.id}`, lang) + '/',
            })))
            : ''}
  ...
  `;
    ...
    return {
        ...
        meta: {
            title,
            description,
            keywords: defaultKeywords(route, lang, [sol.title, cat?.name].filter(Boolean)),
            geoTargets,
            image: sol.img,
            type: 'website',
        },
...
function buildAboutLikeSnapshot(route, lang) {
    const url = buildCanonicalUrl(route, lang);
    const geoTargets = defaultGeoForRoute(route, lang);
    const c = getContent(lang);
    const titles = {
    ...
    const lead = route === 'tour'
        ? t('tourIntro', lang)
        : t('aboutLead', lang);
    const stats = c.COMPANY_STATS;
    const faq = [
        {
            q: lang === 'cn'
                ? '品特工厂在哪里?生产能力多大?'
        ...
        {
            q: t('qMarkets', lang),
            a: (lang === 'cn' ? '品特长期出口至: ' : 'PINTE exports to: ') +
                geoTargets.join(', ') +
                (lang === 'cn'
    ...
    ${stats?.length
        ...
        <ul>${stats
            .map((s) => `<li><strong>${escapeHtml(s.targetValue)}${s.suffix || ''}</strong> — ${escapeHtml(s.label)}</li>`)
            .join('')}</ul>
      </section>`
        : ''}
...
function buildProductsListSnapshot(lang) {
...
function buildFoilsListSnapshot(lang) {
...
function buildQuoteSnapshot(lang) {
    const route = 'quote';
    const url = buildCanonicalUrl(route, lang);
    const geoTargets = defaultGeoForRoute(route, lang);
    const title = lang === 'cn'
...
function buildLegalSnapshot(route, lang) {
    const url = buildCanonicalUrl(route, lang);
    const titles = {
        privacy: {
            cn: '隐私政策 — PINTE 品特烫金箔',
...
function buildBlogListSnapshot(lang) {
    const route = 'blog';
    const url = buildCanonicalUrl(route, lang);
    const geoTargets = defaultGeoForRoute(route, lang);
    const title = lang === 'cn'
...
function buildGuideListSnapshot(lang) {
    const route = 'guides';
    const url = buildCanonicalUrl(route, lang);
    const geoTargets = defaultGeoForRoute(route, lang);
    const title = lang === 'cn'
    ...
    const guides = [...staticGuideCatalogByLang[lang]].sort((a, b) => a.priority - b.priority || a.title.localeCompare(b.title) || a.slug.localeCompare(b.slug));
    const guideLinks = guides
        .map((guide) => `<article><h2><a href="${langPath(`guides/${guide.slug}`, lang)}/">${escapeHtml(guide.title)}</a></h2><p>${escapeHtml(guide.description)}</p></article>`)
        .join('');
    const inner = `
    <h1>${escapeHtml(title)}</h1>
    <p class="seo-lead">${escapeHtml(description)}</p>
...
function buildBlogPostSnapshot(article, lang) {
    ...
    <article>
      ...
      ${article.date ? `<p class="seo-meta"><time datetime="${escapeHtml(article.date)}">${escapeHtml(t('publishedOn', lang))} ${escapeHtml(article.date)}</time></p>` : ''}
      <p class="seo-lead">${escapeHtml(lead)}</p>
    </article>
    ${geoLine(lang, geoTargets)}
  `;
...
function buildSeoSopSnapshot(lang) {
...
function buildPintefoilsSnapshot(lang) {
...
function buildGeoGuideSnapshot(slug, lang) {
    const guide = getGeoGuide(slug);
    if (!guide)
        return null;
    const text = (value) => guideCustomerText(value, lang);
    ...
    const factorRows = guide.factors
        .map((factor) => `<tr><td>${escapeHtml(text(factor.label[lang]))}</td><td>${escapeHtml(text(factor.guidance[lang]))}</td></tr>`)
        .join('');
    const processNotes = guide.processNotes
        ?.map((note) => `<article><h3>${escapeHtml(text(note.title[lang]))}</h3><p>${escapeHtml(text(note.body[lang]))}</p></article>`)
        .join('');
    ...
    const selectionRows = guide.selectionTable
        ?.map((row) => `<tr><td>${escapeHtml(text(row.factor[lang]))}</td><td>${escapeHtml(text(row.confirm[lang]))}</td><td>${escapeHtml(text(row.why[lang]))}</td><td>${escapeHtml(text(row.ask[lang]))}</td></tr>`)
        .join('');
    const substrateRows = guide.substrateFit
        .map((row) => `<tr><td>${escapeHtml(text(row.substrate[lang]))}</td><td>${escapeHtml(text(row.recommendedFoil))}</td><td>${escapeHtml(text(row.note[lang]))}</td></tr>`)
        .join('');
    const troubleshootingRows = guide.troubleshooting
        .map((row) => `<tr><td>${escapeHtml(text(row.issue[lang]))}</td><td>${escapeHtml(text(row.likelyCause[lang]))}</td><td>${escapeHtml(text(row.action[lang]))}</td></tr>`)
        .join('');
    ...
    const researchRows = guide.researchMatrix
        ?.map((row, index) => `<tr><td>${index + 1}</td><td>${escapeHtml(text(row.scenario[lang]))}</td><td>${escapeHtml(text(row.question[lang]))}</td><td>${escapeHtml(text(row.intent[lang]))}</td><td>${escapeHtml(text(row.concern[lang]))}</td><td>${escapeHtml(text(row.sources[lang]))}</td><td>${escapeHtml(text(row.pageType[lang]))}</td><td>${row.conversionScore}</td><td>${row.citationScore}</td><td>${escapeHtml(row.priority)}</td></tr>`)
        .join('');
    ...
    <p><strong>${lang === 'cn' ? '目标读者' : 'Audience'}:</strong> ${escapeHtml(text(guide.audience[lang]))}</p>
    ...
    ${selectionRows
      ...
      <table><thead><tr><th>${lang === 'cn' ? '选型因素' : 'Selection factor'}</th><th>${lang === 'cn' ? '采购前确认' : 'Confirm before buying'}</th><th>${lang === 'cn' ? '为什么重要' : 'Why it matters'}</th><th>${lang === 'cn' ? '询问供应商' : 'Ask your supplier'}</th></tr></thead><tbody>${selectionRows}</tbody></table>
    </section>`
        : ''}
    ...
    ${researchRows
      ...
      <table><thead><tr><th>#</th><th>${lang === 'cn' ? '用途场景' : 'Scenario'}</th><th>${lang === 'cn' ? '问题' : 'Question'}</th><th>${lang === 'cn' ? '意图' : 'Intent'}</th><th>${lang === 'cn' ? '真实采购顾虑' : 'Real buyer concern'}</th><th>${lang === 'cn' ? '常见引用来源' : 'Common sources'}</th><th>${lang === 'cn' ? '页面类型' : 'Page type'}</th><th>${lang === 'cn' ? '转化' : 'Conversion'}</th><th>${lang === 'cn' ? '参考价值' : 'Reference value'}</th><th>${lang === 'cn' ? '优先级' : 'Priority'}</th></tr></thead><tbody>${researchRows}</tbody></table>
    </section>`
        : ''}
    ...
    <section>
      ...
      <table><thead><tr><th>${lang === 'cn' ? '底材' : 'Substrate'}</th><th>${lang === 'cn' ? '推荐系列' : 'Recommended foil'}</th><th>${lang === 'cn' ? '说明' : 'Note'}</th></tr></thead><tbody>${substrateRows}</tbody></table>
    </section>

    <section>
      <h2>${lang === 'cn' ? '常见问题与解决方案表' : 'Common Problems and Fixes'}</h2>
      <table><thead><tr><th>${lang === 'cn' ? '问题' : 'Issue'}</th><th>${lang === 'cn' ? '可能原因' : 'Likely cause'}</th><th>${lang === 'cn' ? '处理方向' : 'Action'}</th></tr></thead><tbody>${troubleshootingRows}</tbody></table>
    </section>
...
function buildGeneratedGuideSnapshot(slug, lang) {
    ...
    const relatedProducts = uniqueNonEmptyValues(guide.relatedProducts).flatMap((id) => {
        const product = getContent(lang).PRODUCT_DATA[id];
        if (!product?.name)
            return [];
        return [{
    ...
    ].map(({ label, value }) => (`<div><dt>${escapeHtml(label)}</dt><dd><time datetime="${escapeHtml(value)}">${escapeHtml(value)}</time></dd></div>`)).join('');
    ...
    <section class="seo-guide-faq" aria-labelledby="guide-faq-title">
      ...
      <dl>${faq.map((item) => (`<dt>${escapeHtml(item.q)}</dt><dd>${escapeHtml(item.a)}</dd>`)).join('')}</dl>
    </section>`;
    ...
    <section class="seo-guide-sources" aria-labelledby="guide-references-title">
      ...
      <ol>${sourceLinks.map((source) => `
        <li>
          <a href="${escapeHtml(source.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(`${source.label}: ${source.title}`)}</a>
    ...
    <article>
      ...
      ${heroImage ? `<figure><img src="${escapeHtml(heroImage)}" alt="${escapeHtml(guide.heroAlt)}">${guide.heroAlt ? `<figcaption>${escapeHtml(guide.heroAlt)}</figcaption>` : ''}</figure>` : ''}
...
// ----------------------------- Public API ----------------------------- //
export function buildSnapshot(route, lang, extras) {
    if (route === '')
        return buildHomeSnapshot(lang);
    if (route === 'about')
        return buildAboutLikeSnapshot('about', lang);
    if (route === 'culture')
        return buildAboutLikeSnapshot('culture', lang);
    if (route === 'tour')
        return buildAboutLikeSnapshot('tour', lang);
    if (route === 'products')
        return buildProductsListSnapshot(lang);
    if (route === 'products/foils')
        return buildFoilsListSnapshot(lang);
    if (route === 'quote')
        return buildQuoteSnapshot(lang);
    if (route === 'privacy')
        return buildLegalSnapshot('privacy', lang);
    if (route === 'terms')
        return buildLegalSnapshot('terms', lang);
    if (route === 'blog')
        return buildBlogListSnapshot(lang);
    if (route === 'guides')
        return buildGuideListSnapshot(lang);
    if (route === 'seo-geo-sop')
        return buildSeoSopSnapshot(lang);
    if (route === 'pintefoils')
        return buildPintefoilsSnapshot(lang);
...
export function renderJsonLdScripts(items) {
    return items
        .map((it) => `<script type="application/ld+json">${JSON.stringify(it).replace(/</g, '\\u003c')}</script>`)
        .join('\n');
}
