...
} from './prerender/snapshot-builder.js';
...
function fixAssetPaths(
  ...
  return html
    ...
    .replace(
      ...
      `<link rel="modulepreload" crossorigin href="${modulePreloadPath}">`
    );
}
...
export const prerender = {
  ...
  async writeBundle() {
    ...
    if (fs.existsSync(assetsDir)) {
      ...
      if (indexFile) indexJsFilename = indexFile;
      if (vendorFile) vendorJsFilename = vendorFile;
      console.log(`✅ Found assets: index=${indexJsFilename}, vendor=${vendorJsFilename}`);
    }

    const languages: Array<'cn' | 'en'> = ['cn', 'en'];

    const productCategoryRoutes = Object.keys(CONTENT_EN.PRODUCT_DATA).map(
      (id) => `products/category/${id}`
    );
    const productItemRoutes = Array.from(
      new Set(
    ...
    const guideRoutes = Array.from(
      new Set([
        ...GEO_GUIDES.map((guide) => `guides/${guide.slug}`),
        ...GENERATED_GUIDES.filter((guide) => guide.status === 'published').map(
